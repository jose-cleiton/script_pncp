...
class PipelineConfig:
    ...
    @property
    def caminho_db_filtrado_gpt(self) -> str:
        return f"data/resultado/{self.data}/pncp_filtrado_gpt.db"
...
class TreinamentoStage:
    ...
    def _treinar(self, X_train: pd.Series, y_train: pd.Series) -> SklearnPipeline:
...
class ClassificacaoStage:
    ...
    def _classificar(self, modelo: SklearnPipeline, df: pd.DataFrame) -> pd.DataFrame:
...
class PncpPipeline:
    ...
    def __init__(self, config: Optional[PipelineConfig] = None) -> None:
        ...
        # GptClassificacaoStage é criado sob demanda em classificar_gpt()
        # para não exigir variáveis de ambiente nas outras etapas.
    ...
    def classificar_gpt(
        ...
        Lê a tabela `contratacoes` do banco SQLite de coleta, envia cada
        `objeto_compra` para o LLM e persiste apenas os registros
        relevantes em um novo banco SQLite.

        Args:
            provedor: ``"openai"`` (padrão) ou ``"gemini"``.
            modelo:   Nome do modelo. Se None, usa o padrão do provedor.
            workers:  Número de workers paralelos. 1 = sequencial (padrão).
